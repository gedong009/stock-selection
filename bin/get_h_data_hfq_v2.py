...
def get_h_data(code, threadName):
    ...
    while start_time < datetime.datetime.now():
        try:
            ...
            stock_data = ts.bar(code, conn, adj="hfq", start_date=str(start_time), end_date=str(end_time),
                                factors=['tor'])
            # p(start_time)
            ...
            #                             'low': 'iMinimumPrice', 'volume': 'iVolume', 'amount': 'iAmount'}, inplace=True)
            stock_data2.rename(columns={'open': 'iOpeningPrice', 'high': 'iMaximumPrice', 'close': 'iClosingPrice',
                                        'low': 'iMinimumPrice', 'vol': 'iVolume', 'amount': 'iAmount',
                                        'code': 'sCode', 'tor': 'iTurnoverRate'}, inplace=True)
...
def get_data(threadName):
    while 1 == 1:
        # 从列表去除
        code = code_redis.get_next_code("new")
        # code = "601313"
        if code:
...
if __name__ == '__main__':
    # 将所有股票代码列入待获取列表中
    code_redis.reset_codelist_redis("new")
    # 建立一个新数组
    threads = []
    # n = 10
    n = 10
